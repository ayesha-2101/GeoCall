...
const LoginPage = () => {
  ...
  useEffect(() => {
    //Here we don't use a useEffect with the if condition, then every time state changes for any variable conditions runs and nake new connection, so by used state it only runs when location changes.
    if (myLocation) {
